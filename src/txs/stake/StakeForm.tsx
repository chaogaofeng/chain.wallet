...
import { AccAddress, Coin, ValAddress } from "@gnchain/chain.js"
import { Delegation, Validator } from "@gnchain/chain.js"
import { MsgDelegate, MsgUndelegate } from "@gnchain/chain.js"
import { MsgBeginRedelegate } from "@gnchain/chain.js"
...
const StakeForm = ({ tab, destination, validators, delegations }: Props) => {
  ...
  const createTx = useCallback(
    ({ input, source }: TxValues) => {
      ...
      const amount = toAmount(input)
      const coin = new Coin("ugnc", amount)
  ...
  /* fee */
  const balance = {
    [StakeAction.DELEGATE]: getAmount(bankBalance, "ugnc"),
  ...
  const token = tab === StakeAction.DELEGATE ? "ugnc" : ""
  const tx = {
    token,
    amount,
    balance,
