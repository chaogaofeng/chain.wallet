...
import { ValAddress } from "@gnchain/chain.js"
...
const ValidatorActions = ({ destination }: { destination: ValAddress }) => {
  ...
  const renderDelegationsValue = () => {
    ...
    const value = calcValue({ amount, denom: "ugnc" })

    return (
      <section>
        <Read amount={amount} denom="ugnc" className={styles.total} block />
        {currency !== "ugnc" && (
          <Read
            amount={value}
            denom={currency}
  ...
  const renderRewardsValue = () => {
    ...
    const amount = list.find(({ denom }) => denom === "ugnc")?.amount ?? "0"

    return (
      <section>
        <Read amount={amount} denom="ugnc" className={styles.total} />{" "}
